...
def test_show_bmp_histogram(image1, dtype):
    image = np.fromfile(image1, dtype)  # 相比于open，该函数可以指定数据类型
    image = image[1078:]
    array_bins = np.arange(0, 256, 1)
    n = plt.hist(image, bins=[0, 16, 32, 48, 64, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 256])
    # n = plt.hist(image, bins=[0, 50, 100, 150, 200, 256])
    # n = plt.hist(image, bins=array_bins)
    # plt.hist(image, bins=array_bins)
    print(n)
    plt.title("histogram")
    plt.show()
